...
TEST_SUITE2(LibraryInterface, AssociatedPrimes)
...
TEST(AssociatedPrimes, Typical) {
  ...
  Frobby::associatedPrimes(ideal, consumer);

  ASSERT_EQ(consumer.getIdeals(), IdealFactory::assoprimes_xx_yy_xz_yz());
}
...
TEST(AssociatedPrimes, ZeroIdeal) {
  for (size_t varCount = 0; varCount <= 3; ++varCount) {
    ...
    Frobby::associatedPrimes(frobbyInputIdeal, consumer);
