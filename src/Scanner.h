...
// This class offers input from a FILE* which is much more convenient and
// for some purposes more efficient than dealing with a FILE* directly.
// It keeps track of the current linenumber to report better error messages,
// so accurate error messages require that ALL input from the file be read
...
public:
  ...
  // Require the next character to be equal to expected. This character
  // is skipped past.
  ...
  // Read an integer and set it to zero if it is negative. This is more
  // efficient because the sign can be detected before the integer
  // is read.
  void readIntegerAndNegativeAsZero(mpz_class& integer);

  // Reads a size_t, where the representable range of that type determines
  // when the number is too big. The number is required to be non-negative.
  ...
  // Reads an identifier and returns the index of that identifier as a
  // variable in names.
  ...
  // Returns true if the next character is whitespace. Does not, obviously,
  // skip whitespace.
  bool peekWhite();
  ...
  // Reads past any whitespace, where whitespace is defined by the standard
  // function isspace().
  void eatWhite();
